...
def test_auth_service_builds_payload_for_authorization_code() -> None:
    oauth = _FakeOAuth()
    svc = AuthService(oauth=oauth, client_id="cid", client_secret="csecret")
    ...
    assert oauth.last_payload is not None
    assert oauth.last_payload["grant_type"] == "authorization_code"
    assert oauth.last_payload["client_id"] == "cid"
    assert oauth.last_payload["client_secret"] == "csecret"
    assert oauth.last_payload["code"] == "abc"
    assert oauth.last_payload["redirect_uri"] == "https://example.com/callback"
...
def test_auth_service_builds_payload_for_refresh_token() -> None:
    ...
    assert oauth.last_payload is not None
    assert oauth.last_payload["grant_type"] == "refresh_token"
    assert oauth.last_payload["refresh_token"] == "r1"
